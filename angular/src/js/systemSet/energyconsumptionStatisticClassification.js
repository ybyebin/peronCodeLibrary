...
function classificationTree(type,results,result,iChecks) {
  ...
  $.each(result.data.items, function(index, item) {
    ...
    // }
    // else {
    html += '<a  href="#' + type + '-s' + item.id + '" data-parent="#accordion" data-toggle="collapse" class="one-step accordion-toggle ' + a + ' pointGroups-a" aria-expanded="true">';
    // }


    html += '<input name="tree-ckss" data-parid="' + type + '-s' + item.id + '"   data-hidetag="'+item.hide_tag+'"   data-step="' + type + '-first" tabindex="5" type="radio" id="' + type + '-' + item.id + '-' + index + '"  class="skin-square-green fu ' + type + '-ckss"  data-id="' + item.id + '" ><label  class="form-label pointGroups-label">' + item.name + '<span data-step="'+type+'-first" data-id="' + type + '-' + item.id + '" data-groupid="' + item.id + '"   class="classification-delete" style="float:right">删除</span></label></a></h4></div></div>';
    html += '<div class="first-body panel-collapse collapse in " id="' + type + '-s' + item.id + '" aria-expanded="true">';
    ...
    if (item.next_level) {
      ...
      $.each(item.next_level, function(index, item) {
        ...
        // } else {
        html += '<a  href="#' + type + '-s' + item.id + '" data-parent="#accordion" data-toggle="collapse" class="accordion-toggle collapsed ' + a + ' pointGroups-a" aria-expanded="false">';
        // }
        html += '<input name="tree-ckss" data-parid="' + type + '-s' + item.id + '" data-hidetag="'+item.hide_tag+'"   data-step="' + type + '-second" tabindex="5" type="radio" id="' + type + '-' + item.id + '-' + index + '"  class="skin-square-green fu ' + type + '-ckss" data-id="' + item.id + '" ><label  class="form-label pointGroups-label">' + item.name + '<span data-step="'+type+'-second" data-id="' + type + '-' + item.id + '"  data-groupid="' + item.id + '"   class="classification-delete" style="float:right">删除</span></label></a></h4></div></div>';
        html += '<div class="second-body panel-collapse collapse " id="' + type + '-s' + item.id + '" aria-expanded="false" style="height: 0px;">';
        ...
        if (item.next_level) {
          ...
          $.each(item.next_level, function(index, item) {
            ...
            html += '<a  href="#' + type + '-s' + item.id + '" data-parent="#accordion" data-toggle="collapse" class="accordion-toggle collapsed ' + a + ' pointGroups-a" aria-expanded="false">';
            // }
            html += '<input name="tree-ckss" data-parid="' + type + '-' + item.id + '"  data-hidetag="'+item.hide_tag+'"  data-step="' + type + '-third" tabindex="5" type="radio" id="' + type + '-' + item.id + '-' + index + '"  class="skin-square-green fu ' + type + '-ckss" data-id="' + item.id + '" ><label  class="form-label pointGroups-label">' + item.name + '<span data-step="'+type+'-third" data-id="' + type + '-' + item.id + '" data-groupid="' + item.id + '"    class="classification-delete" style="float:right">删除</span></label></a></h4></div></div>';
            html += '<div class="thrid-body panel-collapse collapse " id="' + type + '-s' + item.id + '" aria-expanded="false" style="height: 0px;padding-left:45px;">';

            // if (item.hide_tag === 0) {} else if (item.tag_list !== null) {
...
$('#add-samelevemenu-btn').on('click', function() {
  ...
  if ($('.treeclassification-main-body').children().length) {
    ...
    if ($('input[name="tree-ckss"]:checked').hasClass('fu')) {
      ...
      var inpu_oldp = inpu.parent().parent().parent().parent().parent().parent();
      var input_data = {
      ...
      switch (inpu.data('step')) {
        ...
        case 'tree-second':
          ...
          console.log('查看:' + inpu_oldp.find('div.second').length);
          input_data.brother = Number(inpu_oldp.find('div.second').length);
          input_data.groupid = inpu_oldp.attr('id').substring(5);
          // }
          break;
        case 'tree-third':

          console.log('查看:' + inpu_oldp.find('div.third').length);
          if (inpu_oldp.find('div.third').length) {
            input_data.brother = Number(inpu_oldp.find('div.third').length);
          }
    ...
    if (arr.length === 0) {
      ...
      layer.open({
        title: ['选择添加组', 'font-size:18px;color:#fff;background:#3E4687;height:50px;font-weight:bold;line-height:50px;padding-left:30px;border:none;'],
        type: 1,
        ...
        yes: function(layerindex) {
          ...
          if (have) {
            ...
            var urls = apiurl + 'EnergyConfigClassification/1?ids='+arrs.join(',');
            $.ajax({
              url: urls,
              type: 'GET',
              beforeSend: function() {
                ...
                if (result.success) {
                  ...
                  switch (arr[0].type) {
                    case 'tree-first':
                      ...
                      $.each(result.data.items, function(indexnum, item) {
                        console.log('查看一级菜单添加的数据'+JSON.stringify(item,null,2))
                         groups[item.id] = {
                          id: item.id,
                          hide_tag:item.hide_tag,
                          level: 1,
                          higher_level:0 
                        }
                        ...
                        html += '<input name="tree-ckss" data-parid="' + type + '-' + item.id + '" data-hidetag="'+item.hide_tag+'" data-step="tree-first" tabindex="5" type="radio" id="' + type + '-' + item.id + '-' + index + '"  class="skin-square-green fu ' + type + '-ckss"  data-id="' + item.id + '" ><label  class="form-label pointGroups-label">' + item.name + '<span data-step="tree-first" data-groupid="'+ item.id + '" data-id="' + type + '-' + item.id + '"    class="classification-delete" style="float:right">删除</span></label></a></h4></div></div>';
                    ...
                    case 'tree-second':
                      ...
                      $.each(result.data.items, function(indexnum, item) {
                        ...
                        // }
                        html += '<input name="tree-ckss"   data-hidetag="'+item.hide_tag+'" data-step="tree-second" tabindex="5" type="radio" id="' + type + '-' + item.id + '-' + index + '"  class="skin-square-green fu ' + type + '-ckss" data-id="' + item.id + '" ><label  class="form-label pointGroups-label">' + item.name + ' <span data-step="tree-second" data-groupid="'+ item.id + '" data-id="' + type + '-' + item.id + '"   class="classification-delete" style="float:right">删除</span></label></a></h4></div></div>';
                    ...
                    case 'tree-third':
                      ...
                      $.each(result.data.items, function(indexnum, item) {
                        ...
                        html += '<a  href="#' + type + '-' + item.id + '" data-parent="#accordion" data-toggle="collapse" class="accordion-toggle collapsed ' + a + ' pointGroups-a" aria-expanded="false">';
                        // }
                        html += '<input name="tree-ckss"  data-hidetag="'+item.hide_tag+'"  data-step="tree-third" tabindex="5" type="radio" id="' + type + '-' + item.id + '-' + index + '"  class="skin-square-green fu ' + type + '-ckss" data-id="' + item.id + '" ><label  class="form-label pointGroups-label">' + item.name + ' <span data-step="tree-third" data-groupid="'+ item.id + '" data-id="' + type + '-' + item.id + '"   class="classification-delete" style="float:right">删除</span></label></a></h4></div></div>';
                ...
                switch (inpu.data('step')) {
                  case 'tree-first':
                    ...
                    $.each(result.data.items, function(index, item) {                       
                      ...
                      html += '<div class="second panel panel-default ' + type + '-' + item.id + '" ><div class="panel-heading fang" style="margin-left: 15px;padding-top:5px;padding-right:0;"><h4 class="panel-title ">';
                      html += '<a  href="#' + type + '-' + item.id + '" data-parent="#accordion" data-toggle="collapse" class="accordion-toggle collapsed ' + a + ' pointGroups-a" aria-expanded="false">';
                      html += '<input name="tree-ckss" data-hidetag="'+item.hide_tag+'" data-step="tree-second" tabindex="5" type="radio" id="' + type + '-' + item.id + '-' + index_num + '"  class="skin-square-green fu ' + type + '-ckss" data-id="' + item.id + '" ><label  class="form-label pointGroups-label">' + item.name + ' <span data-groupid="' + item.id + '" data-step="tree-second" data-id="' + type + '-' + item.id + '"   class="classification-delete" style="float:right">删除</span></label></a></h4></div></div>';
                  ...
                  case 'tree-second':
                    ...
                    $.each(result.data.items, function(index, item) {
                      ...
                      html += '<a  href="#' + type + '-' + item.id + '" data-parent="#accordion" data-toggle="collapse" class="accordion-toggle collapsed ' + a + ' pointGroups-a" aria-expanded="false">';
                      // }
                      html += '<input name="tree-ckss" data-hidetag="'+item.hide_tag+'" data-step="tree-third" tabindex="5" type="radio" id="' + type + '-' + item.id + '-' + index_num + '"  class="skin-square-green fu ' + type + '-ckss" data-id="' + item.id + '" ><label  class="form-label pointGroups-label">' + item.name + ' <span data-groupid="' + item.id + '" data-step="tree-third" data-id="' + type + '-' + item.id + '"   class="classification-delete" style="float:right">删除</span></label></a></h4></div></div>';
                      html += '<div class="thrid-body panel-collapse collapse " id="' + type + '-' + item.id + '" aria-expanded="false" style="height: 0px;padding-left:45px;">';
    ...
    layer.open({
      ...
      yes: function(index) {
        ...
        $('input[name="alert-ckss"]:checked').each(function(index, element) {
          ...
          arr.push(Number($(element).attr('id')));
        });
        console.log('上传的数据:'+JSON.stringify(arr,null,2));
        var urls = apiurl + 'EnergyConfigClassification/1?ids='+arr.join(',');
        if (arr.length === 0) {
          layer.msg('未选择能耗组');
        }else{
           $.ajax({
          ...
          success: function(result) {
            ...
            if (result.success) {
              ...
              $.each(result.data.items, function(index, item) {
                ...
                html += '<input name="tree-ckss" data-parid="' + 'tree' + '-' + item.id + '"  data-hidetag="' + item.hide_tag + '" data-step="' + 'tree' + '-first" tabindex="5" type="radio" id="' + type + '-' + item.id + '-' + index + '"  class="skin-square-green fu ' + type + '-ckss"  data-id="' + item.id + '" ><label  class="form-label pointGroups-label">' + item.name + '<span data-groupid="0" data-step="tree-first" data-id="' + type + '-' + item.id + '"    class="classification-delete" style="float:right">删除</span></label></a></h4></div></div>';
...
function loadGroups(str) {
  $.ajax({
    url: apiurl + "energyconfig?name="+str,
    type: 'GET',  
    beforeSend: function() {
      $(".loading").show();
    },
  ...
  $('input[data-step="tree-first"]').each(function(index, element) {
    ...
    $('#' + $(element).data('parid') + ' input[data-step="tree-second"]').each(function(indexs, elements) {
      ...
      dic[$(elements).data('id')] = {
         id: $(elements).data('id'),
         hide_tag:$(elements).data('hidetag'),
         level: 2,
