    ...
    success: function(data) {
      ...
      if (data.success) {
        ...
        if (data.data[0].view.length !== 0) {
          $('#wraplist li:first-child ul li:first-child a').click();
          // view_id = data.data[0].view[0].viewId;
          // MQTTconnect();
...
function addSlide(datagroups, i, str) {
  ...
  var li = '<li class="device-li" data-toggle="collapse" data-target="#demo-li' + i + '" aria-expanded="false">' + '<a><i class="' + str + '"></i><span>' + datagroups[i].groupName + '</span><i class="iarrow"></i></a>' + '<ul class="collapse in" id="demo-li' + i + '">' + ul_son + '</ul></li>';
  $('#wraplist').append(li);
}

$('#wraplist').on('click', 'li ul li', function(e) {
  if (e && e.stopPropagation) {
    e.stopPropagation();
  }
  $('#wraplist li ul li').removeClass('active');
  $(this).addClass('active');
  var stringId = $(this).find('a').data('viewid');
    ...
    success: function(data) {
      ...
      if (data.success) {
        ...
        for (var i in subCanvas) {
          ...
          var obj = '<p><button class="btn boolean-true" id="' + subCanvas[i].id + '" data-tag-id="' + subCanvas[i].tag_id + '" data-tag-name="' + subCanvas[i].tag_name + '" data-tag-type="' + subCanvas[i].tag_type + '" data-bingding-status="' + subCanvas[i].bingding_status + '" data-readonly="' + subCanvas[i].readonly + '" data-name="' + subCanvas[i].name + '">' + subCanvas[i].name + '</button></p>';
        ...
        if (data.data.background_img_url !== null) {
...
$('#SecCanvasFullBtn').on('click', 'button', function() {
  console.log('按钮');
  console.log('查看:' + $(this).data('tag-id'));
  if ($(this).data("tag-id") !== "") {
    console.log('123')
        ...
        switch (componentIsReadonly[btnId].tag_type) {
          case 'boolean':
            layerType = layer.open({
              title: ['指定数据标签值', 'font-size:18px;color:#333;background:#fff;height:50px;font-weight:bold;line-height:50px;padding-left:30px'],
              type: 1,
          ...
          case 'string':
            ...
            layerType = layer.open({
              title: ['制定数据标签值', 'font-size:18px;color:#333;background:#fff;height:50px;font-weight:bold;line-height:50px;padding-left:30px'],
              type: 1,
              skin: 'layui-primary', //加上边框
              area: ['1018px', '275px'], //宽高
...
function detail(title) {
  layer.open({
    title: [title, 'font-size:18px;color:#333;background:#fff;height:50px;font-weight:bold;line-height:50px;padding-left:30px'],
    type: 1,
    area: ['1536px', '600px'], //宽高
  ...
  $.ajax({
    ...
    success: function(data) {
      if (data.success) {
        ...
        layer.msg("获取数据失败:" + data.error_message)
        console.log("获取数据失败" + JSON.stringify(data, null, 2))
      }
    },
    error: function(data) {
      layer.msg("获取数据失败:" + data.error_message)
...
function changeComponentState(componentID) {
  ...
  if (componentIsReadonly[componentID]) {
    ...
    // if (Number(componentIsReadonly[componentID].readonly) === 0 && componentIsReadonly[componentID].com_readonly == false)
    // {
    //   isReadOnly = 0;
    // }
  }
