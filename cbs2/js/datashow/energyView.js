...
$(document).ready(function() {
  ...
  initScrollerA("leftNames");     
  //树动态绑定事件
  $("#tree input.skin-square-green").on('ifUnchecked ifChecked', function() {
    checkTree('tree');
...
function showObj(type) {
  if (type == 1) {
    if ($("#tongji").is(":hidden")) {
      $("#tongji").slideDown(300);
    } else {
      $("#tongji").slideUp();
    }
  } else {
    if ($("#tongjis").is(":hidden")) {
      $("#tongjis").slideDown(300);
    } else {
      $("#tongjis").slideUp();
    }
  }
...
//删除div
function del(type) {
  layer.confirm('<h4>您确认要删除该项目？</h4><p><small>删除后不可恢复</small></p>', {
    btn: ['取消', '删除'], //按钮
    area: ['1000px'], //宽高
    shade: 0,
    title: false,
...
function otherSave(type, name, string) {
  ...
  layer.open({
    title: ['' + string + '', 'font-size:18px;color:#333;background:#fff;height:50px;font-weight:bold;line-height:50px;padding-left:30px'],
    type: 1,
        ...
        if (!status) {
          ...
          } else if (type == 2) //能耗报表
          {
            if (fileName_s("fieldName", 2)) {
              ...
              _r = "api/energy/report/create"; //创建
              $.ajax({
                type: "post",
                url: enetgy_url + _r + '',
                dataType: 'json',
...
function compares() {
  ...
  if ($("#selectTime").val() != "" && $("#endTime").val() != "") {
    ...
    var index = layer.open({
      title: ['对比历史数据', 'font-size:18px;color:#333;background:#fff;height:50px;font-weight:bold;line-height:50px;padding-left:30px'],
      type: 1,
    ...
    // $(".layui-compare .layui-layer-close1").click(function() {
    //     $(".addTime").html("");
    // })
  } else {
    layer.msg("时间不能为空");
...
function loadHighcharData(screening, id, tag_taggroup, com) {
  ...
  switch (screening) {
    case "screening":
      $.each($("#tree input[class*='skin-square-green']:checked"), function(index, item) {
        tagId = $(this).attr("ids");
  ...
  if (tagId != null) {
    ...
    var _r = "api/energy/graph/history";
        ...
        if (charname == 'pie') {
          if (result.success) {
            ...
            if (result.success) {
              ...
              for (var keypie in result.data) {
                seriesdata += '{name:"' + keypie + '",y:' + result.data[keypie] + ',color:"' + colors[j] + '"},';
                j++;
              }
          ...
          if (result.success) {
            ...
            for (var key in result.data) {
              returndata += "{name:'" + key + "',marker:{enabled:true,radius:3},color:'" + colors[i] + "',data:[";
              areadata += "{name:'" + key + "',color:'" + colors[i] + "',data:[";
              var datename = "";

              if (!isEmptyObject(result.data[key])) {
                for (var keys in result.data[key]) {
                  switch (chardatetype) {
                    case "hour":
                  ...
                  returndata += "[" + datestr + "," + result.data[key][keys] + "],";
                  areadata += "[" + datestr + "," + result.data[key][keys] + "],";
...
function selectDrown(obj, type) {
  ...
  // var he = $("#compare1").parent().height();
  // he = he + 290;
  // $("#compare1").parent().height(he);

  $(".downselect .dropList" + obj + " li a").one("click", function() {

    var txt = $(this).text();
    var value = $(this).attr("rel");
    //var times=compareDate(value);

    //获取下拉框选择时间 用当前时间减去它
    // initAdd(type, obj, value);
...
function checkTree(name, type) {
  var arr = new Array();
  var ids = ""; //选择的框的ID值
  var b = $("#" + name + " input[class*='skin-square-green']:checked").length;
  if (!type) {
  ...
  if (b > 64) {
    $("#" + name + " .icheckbox_square-green']").on('ifUnchecked', function(event) {
      $("#" + name + " input[class*='skin-square-green']").iCheck('uncheck');
    });
  }
  $.each($("#" + name + " input[class*='skin-square-green']:checked"), function(index, item) {
    ids += $(this).attr("id") + ",";
  })
  arr[0] = b;
  arr[1] = ids;
...
function highlight(ida, idb) {
  ...
  //$(".tree .skin-square-green").iCheck("uncheck");
...
function leftList(type) {
  var _r = "";
  if (type == 1) {
    _r = "api/energy/graph/list";
  } else if (type == 2) {
    _r = "api/energy/report/list";
  }
  $.ajax({
    type: "GET",
    url: enetgy_urls,
...
function clearCheck(type) {
  if (type == 1) {
    $('#tree input').iCheck('uncheck');
    clearSelection('tree');
    $("#history").removeClass("btnborder_disabled").removeAttr('disabled');
  } else {
    $('#trees input').iCheck('uncheck');
      ...
      if (results.success) {
        $.ajax({
          ...
          success: function(result) {
            ...
            //     // console.log("组+++++++++++")
            //     // alert(result.data);
            //     a = item.tag_id_tree ? "add" : "";
            //     html += '<div class="panel panel-default"><div class="panel-heading fang"><h4 class="panel-title ">';
            //     if (item.hide_tag) {
            //       html += '<a  href="#' + ids + '-' + item.id + '" data-parent="#accordion"  class="accordion-toggle collapsed ' + a + ' pointGroups-a" aria-expanded="false">';
            //     } else {
            //       html += '<a  href="#' + ids + '-' + item.id + '" data-parent="#accordion" data-toggle="collapse" class="accordion-toggle collapsed ' + a + ' pointGroups-a" aria-expanded="false">';
            //     }
            ...
            //         html += '<div class="panel-collapse collapse " id="' + ids + '-' + item.id + '" aria-expanded="false" style="height: 0px;">';
            //         $.each(tags, function(indexs, items) {
            //           html += '<div class="panel-body tree-msg"><a href="#' + ids + '-' + items.id + '" data-parent="#accordion" data-toggle="collapse" class="accordion-toggle collapsed pointGroups-a-son" aria-expanded="false"><input tabindex="5" oprate="' + items.oprate + '" type="checkbox" id="' + ids + '-' + item.id + '-' + index + '-' + indexs + '" class="skin-square-green" ids="' + items.tag_id + '" ><label class="form-label pointGroups-label-son">' + items.name + '</label></a></div>';
            //         })
            ...
            //     $('#' + ids + ' input.skin-square-green').iCheck({
            ...
            if (result.success) {
              ...
              $.each(result.data, function(index, item) {
                ...
                if (a === '') {
                  html += '<a  href="#' + ids + '-' + item.id + '" data-parent="#accordion"  class="one-step accordion-toggle  ' + a + ' pointGroups-a" aria-expanded="true">';
                } else {
                  html += '<a  href="#' + ids + '-' + item.id + '" data-parent="#accordion" data-toggle="collapse" class="one-step accordion-toggle  ' + a + ' pointGroups-a" aria-expanded="false">';
                }


                html += '<input tabindex="5" type="checkbox" id="' + ids + '-' + item.id + '-' + index + '"  class="skin-square-green fu" ids="' + item.id + '" ><label  class="form-label pointGroups-label">' + item.name + '</label></a></h4></div></div>';
                html += '<div class="panel-collapse collapse in" id="' + ids + '-' + item.id + '" aria-expanded="true">';
                  ...
                  $.each(item.next_level, function(index, item) {
                    ...
                    if (item.next_level) {
                      ...
                      $.each(item.next_level, function(index, item) {
                        ...
                        html += '<input tabindex="5" type="checkbox" id="' + ids + '-' + item.id + '-' + index + '"  class="skin-square-green fu" ids="' + item.id + '" ><label  class="form-label pointGroups-label">' + item.name + '</label></a></h4></div></div>';
                        html += '<div class="panel-collapse collapse " id="' + ids + '-' + item.id + '" aria-expanded="false" style="height: 0px;padding-left:45px;">';
...
function Del(type) {
  ...
  var _r;
  if (type == 1) {
    _r = "api/energy/graph/delete"; //删除
  } else {
    _r = "api/energy/report/delete"; //删除
  }
  $.ajax({
    type: "post",
    url: enetgy_url + _r + '',
    ...
    success: function(result) {
      ...
      if (result.success) {
        if (type == 1) {
          ...
          $("#" + names + " input[class*='skin-square-green']").iCheck('uncheck'); //取消之前选中的
        } else {
          $("#bao_leftNames aside").remove("aside");
          leftList(2);
          $("#bao_leftNames aside li").removeClass("active");
    ...
    error: function(data) {
      $(".loading").hide();
      layer.msg("删除失败:" + data.error_message);
    }
  })
}
...
//  能源分析保存、更新获取参数
function getdata(v) {
  var type, lengths, length_unit = null,
    start_time = null,
    end_time = null,
    relative_history = "[]",
    absolute_history = "[]";
...
function save(type) {
  ...
  if (type == 1) {
    ...
    if (!$("#leftNames aside li").hasClass("active")) {
      ...
      _r = "api/energy/graph/update";
      $.ajax({
        type: "post",
        url: enetgy_url + _r + '',
        dataType: 'json',
        data: {
          data: JSON.stringify(getdata(1))
        },
        success: function(result) {
          if (result.success) {
    ...
    if (!$("#bao_leftNames aside li").hasClass("active")) {
      ...
      if (!status) {
        $.ajax({
          ...
          dataType: 'json',
          data: {
            data: JSON.stringify(reportData(1))
          },
          success: function(result) {
            if (result.success) {
              layer.msg("更新成功");
...
function reportData(v) {
  ...
  $.each($("#trees input[class*='skin-square-green']:checked"), function(index, item) {
    if ($(this).hasClass("fu")) {
      var par = $(this).attr("id").split("-");
      var pname = $(this).parent().next().html();
      // tag_id_tree.push(new Objfu($(this).attr("ids"), pname, 1, null));
...
function check_tree(type, ids) {
  ...
  var datas, _r;
  if (type == 1) {
    _r = "api/energy/graph/get";
    datas = {
      id: ids,
      r: _r
    }
  } else {
    _r = "api/energy/report/get";
    datas = {
      id: ids,
      r: _r
    }
    ...
    success: function(result) {
      ...
      if (result.success) {
        ...
        if (result.data.tag_id_tree != null) {
          $.each(result.data.tag_id_tree, function(index, item) {
            var selected = item.selected;
            if (selected == 1) //父元素被选中
            {
              $("a[href='#" + names + "-" + item.id + "']").find(".skin-square-green").iCheck('check');

            }
            if (item.tags) //有子类
            {
              //父类展开
              $("#" + names + "-" + item.id + "").addClass("in").css("height", "");
              $.each(item.tags, function(index, items) {
                ...
                $("#" + names + "-" + item.id + "").find("input[ids='" + items.id + "']").iCheck('check');
              })
        ...
        if (type == 1) {
          ...
          if (result.data.type == 1) //相对时间
          {
            ...
            $("#selectTime").val(result.data.start_time);
          ...
          // 
          // 
          if (result.data.absolute_history.length != 0 && result.data.relative_history.length == 0) {
            getHistory(result.data.absolute_history, 1, result.data.length_unit);
          }
          if (result.data.relative_history.length != 0 && result.data.absolute_history.length == 0) {
            getHistory(result.data.relative_history, 2, result.data.length_unit);
          }
          ...
          if (result.data.absolute_history.length != 0 || result.data.relative_history.length != 0) {
            // alert("历史数据对比")
            isHistoryData = true;
            if (result.data.tag_id_tree.length == 0) {
              $("#lineCharts").html("");
              var spanwarn = '<span id="sTitle" class="sTitle">统计对象为空</span>'
              $("#lineCharts").append(spanwarn);
            } else {
              if (result.data.absolute_history.length != 0) {

                if (result.data.tag_id_tree[0].selected == 0) {
                  SelectDataHistory(1, result.data.tag_id_tree[0].tags[0].id, 1, "compare"); // 1:单独tag. compare:绝对时间
                } else {
                  // alert("组节点")
                  SelectDataHistory(1, result.data.tag_id_tree[0].id, 2, "compare"); // 2:tagGroup. compare1:相对时间
                }


              } else {
                if (result.data.tag_id_tree[0].selected == 0) {
                  SelectDataHistory(1, result.data.tag_id_tree[0].tags[0].id, 1, "compare1"); // 1:单独tag. compare:绝对时间
                } else {
                  SelectDataHistory(1, result.data.tag_id_tree[0].id, 2, "compare1"); // 2:tagGroup. compare1:相对时间
                }
          ...
          switch (stype) {
            ...
            case "month":
              ...
              $("#bao_month").val(time).show();
              break;
            case "year":
              $("#selectTypes .txt").attr("rel");
              $("#selectTypes .txt").html("年报").attr("rel", "3");
              $("#bao_year").val(time).show();
              break;
          }
          if (result.data.tag_id_tree.length != 0) {
            BaoTableList(); //绑定表格数据
...
function initData(type) {
  if (type == 2) {
    ...
    if (!lowd) //没有加载过
    {
      ...
      $("#trees input.skin-square-green").on('ifUnchecked ifChecked', function() {
        checkTree('trees', 1);
      });
...
function BaoTableList() {
  ...
  var _r = "api/energy/report/table";
  $.ajax({
    type: "post",
    url: enetgy_url + _r + '',
    dataType: 'json',
      ...
      if (result.success) {
        ...
        if (dataIsNull) {
          ...
          for (var key in result.data) {
            html += "<tr><td>" + key + "</td>";
            for (var keys in result.data[key]) {
              html += "<td>" + result.data[key][keys] + "</td>";
            }
            html += "</tr>";
          }
...
function tttt(result) {
  ...
  $.each(result.data, function(index, item) {
    ...
    html += '<div class="panel panel-default"><div class="panel-heading fang" style="padding-left:0;padding-right:0px;" ><h4 class="panel-title">';
    if (a === '') {
      html += '<a  href="#' + ids + '-' + item.id + '" data-parent="#accordion"  class="one-step accordion-toggle collapsed ' + a + ' pointGroups-a" aria-expanded="false">';
    } else {
      html += '<a  href="#' + ids + '-' + item.id + '" data-parent="#accordion" data-toggle="collapse" class="one-step accordion-toggle collapsed ' + a + ' pointGroups-a" aria-expanded="false">';
    }
  ...
  console.log("查看:" + html)
  $("#tree").append(html);
  // $("#tree" + " .mCSB_container").append(html);
  // $("#tree" + "").mCustomScrollbar("scrollTo");
  ULTRA_SETTINGS.iCheck();
