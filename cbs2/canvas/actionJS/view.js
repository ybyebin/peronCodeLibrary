...
example.Toolbar = Class.extend({
	...
	init: function(elementId, view) {
		...
		// 保存按钮
		this.saveButton = $('<label class = "toolbar-save"><img class=" baocun-img " src="img/componentImg/baocun.png"><br>保存</label>');
					...
					if (json.length == 0) {
						...
						$('.have-btn button').each(function(index, element) {
							...
							var dic = {
								...
								tag_type: $(element).data('tag-type'),
								tag_name: $(element).data('tag-name'),
								bingding_status: $(element).data('bingding-status'),
						...
						// 此处上传用户设置好的数据
						$.ajax({
							url: apiurl + 'r=api/entity/view/edit',
							type: 'post',
							dataType: 'json',
							data: {
		...
		// 添加删除按钮
		this.deleteButton = $('<label class = "toolbar-delete"><img class=" delete-img " src="img/componentImg/delete.png"><br>删除</label>');
		...
		// 清除画布
		this.clearButtons = $('<label class = "toolbar-wangge"><img class=" wangge-img " src="img/componentImg/clear.png"><br>全部清除</label>');
		...
		this.clearButtons.button().click($.proxy(function() {
			...
			layer.open({
				...
				yes: function(index) {
					...
					$("#myBgimage").attr("src", "img/logo-text.png"); //清除用户上传的图片
		...
		// 添加撤销按钮及回调
		//																		
		this.undoButton = $('<label class = "toolbar-undo"><img class=" chexiao-img " src="img/componentImg/chexiao.png"><br>撤销</label>');
		...
		// 添加重做按钮及回调
		//
		this.redoButton = $('<label class = "toolbar-redo"><img class=" fanhui-img " src="img/componentImg/fanhui.png"><br>恢复</label>');
