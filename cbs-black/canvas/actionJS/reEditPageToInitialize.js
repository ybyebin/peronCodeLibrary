...
function getGroupNameAndViewName(id,canvas) {
	...
	console.log("上传的数据:" + JSON.stringify(dataUpGet, null, 2))

	$.ajax({
		url: apiurl + 'r=api/entity/view/init',
		type: 'post',
		dataType: 'json',
		data: {
			data: JSON.stringify({view_id: id})
		},
		beforeSend: function() {
			$(".loading").show();
		},
		...
		success: function(data) {
			...
			if (data.success) {
				...
				// 此处添加  头部的  系统名称和 画面名称
				$('.go-subsys span').text(data.data.sys_name);
				$('.group-name').text(data.data.view_name);
				if (data.data.background_img_url === null || data.data.background_img_url==='') {
				}else{
					$("#myBgimage").attr("src", data.data.background_img_url);
					$("#myBgimage").data("url", data.data.background_img_url);
				}
				...
				var canvasJson = data.data.view_data.canvas;
				var subCanvas = data.data.view_data.subCanvas;
				...
				for (var i in subCanvas) {
					...
					$('.have-btn #mCSB_3_container').append(obj);
				}
