...
$('#comp-width').on('input', function() {
	...
	if (getNode()) {
		...
		// if (Number($(".conventional-width").val()) <= 1275 - node.getAbsoluteX() && $(".conventional-width").val() !== "") {
			node.setWidth($('#comp-width').val());
			node.repaint();
		// } else {
			// layer.msg("控件超出画面,请重新填写");
		// }
...
$('#comp-height').on('input', function() {
	...
	if (getNode()) {
		...
		// if (Number($('#comp-height').val()) <= 694 - node.getAbsoluteY() && $(".conventional-height").val() !== "") {
...
$('#comp-tagaddress').on('input', function() {
	console.log("输入框的值:" + $('#comp-tagaddress').val())
	if (getNode()) {
		var node = getNode();
		$.ajax({
			...
			success: function(data) {
				$('.loading').hide();
				if (data.success) {
					node.userData.Tag.tag_type = data.data.tag_type;
					...
					$("#tagWrongImg").show();
					$("#tagWrongImg").attr("src", "../canvas/img/right.png");
				} else {
					...
					$("#tagWrongImg").show();
					$("#tagWrongImg").attr("src", "../canvas/img/worry.png");
	...
	layer.open({
		...
		yes: function(index) {
			...
			if ($('input[name="search_ckss"]:checked').data('id') === null) {
				...
				var node = getNode();
				node.userData.Tag.tag_type = $('input[name="search_ckss"]:checked').data('type');
				node.userData.Tag.tag_id = $('input[name="search_ckss"]:checked').data('id');
				node.userData.Tag.tag_name = $('input[name="search_ckss"]:checked').data('name');
				node.userData.Tag.bingding_status = 1;
...
function showPreview(source) {
	...
	if (window.FileReader) {
		...
		fr.onload = function(e) {
			...
			$.ajax({
				url: apiurl + 'r=api/upload/uploadimage',
				type: 'post',
				dataType: 'json',
				data: {
				...
				success: function(data) {
					...
					if (data.success) {
						...
						layer.msg('图片上传成功');
						$('#myBgimage').data("url", "/" + data.data.filename);
						$('#myBgimage').attr("src", e.target.result);
						$('#canvas-img').val(data.data.filename);

					} else {
						layer.msg("上传图片失败:" + data.error_message);
						console.log("上传图片失败原因:" + JSON.stringify(data, null, 2))
					}
				},
				error: function(data) {
					$('.loading').hide();
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (data.success) {
						...
						layer.msg('图片上传成功');
						$('#style-image').val("/" + data.data.filename);
						node.userData.picture = "/" + data.data.filename;
		...
		fr.onload = function(e) {
			...
			$.ajax({
				...
				success: function(data) {
					$('.loading').hide();
					if (data.success) {
						// console.log("上传图片成功onTrue:" + JSON.stringify(data, null, 2))
						layer.msg('图片上传成功');
						$('#ontrue-image').val("/" + data.data.filename);
						node.userData.onTrue.picture = "/" + data.data.filename;
					} else {
						layer.msg("上传图片失败:" + data.error_message);
						// console.log("上传图片失败原因:" + JSON.stringify(data, null, 2))
					}
		...
		fr.onload = function(e) {
			...
			$.ajax({
				...
				success: function(data) {
					$('.loading').hide();
					if (data.success) {
						// console.log("上传图片成功onFalse:" + JSON.stringify(data, null, 2))
						layer.msg('图片上传成功');
						$('#onfalse-image').val("/" + data.data.filename);
						node.userData.onFalse.picture = "/" + data.data.filename;
					} else {
						layer.msg("上传图片失败:" + data.error_message);
						// console.log("上传图片失败原因:" + JSON.stringify(data, null, 2))
					}
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (data.success) {
						...
						layer.msg('图片上传成功');
						$('#onalarm-image').val("/" + data.data.filename);
						node.userData.onAlarm.picture = "/" + data.data.filename;
					} else {
						layer.msg("上传图片失败:" + data.error_message);
						console.log("上传图片失败原因:" + JSON.stringify(data, null, 2))
					}
		...
		fr.onload = function(e) {
			...
			$.ajax({
				...
				success: function(data) {
					$('.loading').hide();
					if (data.success) {
						console.log("上传图片成功OnDisconnected:" + JSON.stringify(data, null, 2))
						layer.msg('图片上传成功');
						$('#onDisc-image').val("/" + data.data.filename);
						node.userData.onDisconnected.picture = "/" + data.data.filename;
					} else {
						layer.msg("上传图片失败:" + data.error_message);
						console.log("上传图片失败原因:" + JSON.stringify(data, null, 2))
					}
...
$('.span-add').on("click", function() {
	...
	iForAdd += 1;
	var obj = '<p><button id="' + id + '" data-tag-id="" data-tag-name="" data-tag-type="" data-bingding-status="0" data-readonly="false" data-name="按钮">' + '按钮</button><img src="img/delete.png"></p>';
	$('.have-btn #mCSB_3_container').prepend(obj);
...
$('.have-btn').on('click', 'img', function() {
	$(this).parent().remove();
	if ($('.have-btn #mCSB_3_container').html() === "") {
		$('.no-glo-btn').show();
	}
});
// 全局按钮点击方法
$('.have-btn').on('click','button',function(){
	...
	switch (Number($(this).data('bingding-status'))) {
		...
		case 1:
			$("#glo-btn-img").show();
			$("#glo-btn-img").attr("src", "../canvas/img/right.png");
			break;
		case 2:
			$("#glo-btn-img").show();
			$("#glo-btn-img").attr("src", "../canvas/img/worry.png");
			break;
	}
		...
		$.ajax({
			...
			error: function(data) {
				$('.loading').hide();
					...
					obj.data('bingding-status',2);							
					$("#glo-btn-img").show();
					$("#glo-btn-img").attr("src", "../canvas/img/error.png");
					console.log("绑定tag失败------失败原因:" + JSON.stringify(data, null, 2))
			}
		});
	}else{
