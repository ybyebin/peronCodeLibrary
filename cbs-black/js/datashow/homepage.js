	...
	$.ajax({
		...
		success: function(data) {
			...
			if (data.success) {
				...
				// for (var key in data_highchar.data) {
				// 	var group = [];
				// 	// var timestamp = Date.parse(new Date(data_highchar.data[key].time)) / 1000;
				// 	// group.push(timestamp);
				// 	group.push(data_highchar.data[key].time);
				// 	group.push(data_highchar.data[key].value);
				// 	data_group.push(group);
				// }
				// console.log(JSON.stringify(data_group,null,2));
				// if (data_group.length == 0) {
				// 	data_group = [
				// 		[
				// 			1487655425443,
