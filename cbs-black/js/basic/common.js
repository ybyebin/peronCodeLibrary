...
function compareDate(type, counts) {
  ...
  if (type == 1) {
    //现在时间减去天数
    mydate = CutDays(parseDate(FormatDate(nowdate, "yyyy-MM-dd")), counts);

    return FormatDate(mydate, "yyyy-MM-dd");

  } else if (type == 2) {
    //现在时间减去月数
    mydate = CutMOnth(parseDate(FormatDate(nowdate, "yyyy-MM-dd")), counts);
...
function compareYear(start, counts) {
  mydate = AddYear(parseDate(FormatDate(start, "yyyy-MM-dd")), counts);
  return FormatDate(mydate, "yyyy-MM-dd");
